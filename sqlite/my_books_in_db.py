...
# 테이블 생성용 함수
def create_table():
    conn = sqlite3.connect('')  # 데이터베이스 커넥션 생성
 #   conn = sqlite3.connect('C:/pycharmProject/Sources/basic01/sqlite/my_books.db')  # 데이터베이스 커넥션 생성
    ...
    # my_books 테이블 생성
    cur.execute('''CREATE TABLE tbl_API (
                        num integer,
                        title text,
                        regdate text
                )'''
